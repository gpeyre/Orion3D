...
*/
/*------------------------------------------------------------------------------*/
void GW_Vertex::BuildFromFile( GW_Ifstream& file)
{
	file >> Position_ >> Normal_ >> CurvDirMin_ >> CurvDirMax_
		>> rMinCurv_ >> rMaxCurv_
		>> TexCoords_[0] >> TexCoords_[1] >> nID_;
	GW_I32 nTempId;
	file >> nTempId;
	if( nTempId>=0 )
	{
...
*/
/*------------------------------------------------------------------------------*/
void GW_Vertex::BuildToFile( GW_Ofstream& file )
{
	file << Position_ << Normal_ << CurvDirMin_ << CurvDirMax_
		<< rMinCurv_ << rMaxCurv_
		<< TexCoords_[0] << TexCoords_[1] << nID_;
	if( pFace_!=NULL )
	{
		GW_I32 nID = pFace_->GetID();
		file << nID;
	}
	else
	{
		/* a vertex not referenced by any face ... */
		GW_I32 nID = -1;
		file << nID;
	}
}
