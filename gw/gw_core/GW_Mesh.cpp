...
/*------------------------------------------------------------------------------*/
// Name : GW_Mesh::BuildFromFile
/**
*  \param  file [ifstream&] The file.
*  \author Gabriel Peyré
*  \date   4-2-2003
* 
...
*/
/*------------------------------------------------------------------------------*/
void GW_Mesh::BuildFromFile( GW_Ifstream& file)
{
	this->SetStaticThis();

	GW_U32 nNbrVertex, nNbrFace;
	file >> nNbrVertex >> nNbrFace;
	this->SetNbrVertex( nNbrVertex );
	this->SetNbrFace( nNbrFace );
	/* first create face pointers */
	for( GW_U32 i=0; i<nNbrFace; ++i )
	{
		GW_Face* pFace = &this->CreateNewFace();
		this->SetFace( i, pFace );
	}
	/* vertex needs to make reference to these faces */
	for( GW_U32 i=0; i<nNbrVertex; ++i )
	{
		GW_Vertex* pVert = &this->CreateNewVertex();
		this->SetVertex( i, pVert );
		file >> *pVert;
	}
	/* then import faces */
	for( GW_U32 i=0; i<nNbrFace; ++i )
...
*/
/*------------------------------------------------------------------------------*/
void GW_Mesh::BuildToFile( GW_Ofstream& file )
{
	GW_U32 nNbrFace   = this->GetNbrFace();
	GW_U32 nNbrVertex = this->GetNbrVertex();
	file << nNbrVertex << nNbrFace;
	for( IT_VertexVector it=VertexVector_.begin(); it!=VertexVector_.end(); ++it)
	{
		GW_Vertex* pVert = *it;
