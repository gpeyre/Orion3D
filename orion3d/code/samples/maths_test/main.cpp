...
{
	OR_U32 i;
	OR_Float rTotalDif = 0;
	OR_U64 nTime;

	/* check the gauss seidel code */
	printf("\n----- Gauss Seidel test -----\n");
	#define SYSTEM_SIZE 1000
	OR_MatrixNxP M;
	OR_VectorND x;
	OR_VectorND b;
	M.Reset(SYSTEM_SIZE,SYSTEM_SIZE);
	x.Reset(SYSTEM_SIZE);
	b.Reset(SYSTEM_SIZE);
	M.Randomize();
	b.Randomize();
	...
	for( i=0; i<SYSTEM_SIZE; ++i )
		M.SetData(i,i, OR_RAND + 10);

	for( i=0; i<50; ++i )
	{
		OR_Float err = (M*x - b).NormInf();
		printf("error : %f\n", err);
		M.GaussSeidelStep(x, b);
	}

	

	/* check the matrix inversion code */
	printf("\n----- OR_Matrix4x4 test -----\n");
	nTime = OR_GetTime();
	for( i=0; i<NBR_ITERATION; ++i )
	{
		...
		m.Randomize();
		m = m*m.Invert();
		OR_Float rDif = (OR_Matrix4x4() - m).NormInf();
		if( rDif>OR_EPSILON )
			printf("Error in inversion of %f \n", rDif);
		rTotalDif += rDif;
	}
	nTime = OR_GetTime() - nTime;
	printf("- Number of iterations : %i\n", NBR_ITERATION);
	printf("- Total time : %u miliseconds = %f seconds\n", 0, ((OR_I64)nTime)*0.001f);
	printf("- Difference to identity : %f, mean : %f\n", rTotalDif, rTotalDif/NBR_ITERATION);
	...
	for( i=0; i<NBR_ITERATION; ++i )
	{
		...
		m.Randomize();
		m = m*m.Invert();
		OR_Float rDif = (OR_Matrix3x3() - m).NormInf();
		if( rDif>OR_EPSILON )
			printf("Error in inversion of %f \n", rDif);
		rTotalDif += rDif;
	}
...
int main( int argc, char *argv[ ], char *envp[ ] )
{
	...
	GlWin=pInputOutput->Initialisation_Win32(NULL,NULL,0,"-==[ Test for maths ]==-",640,480,0,0);
