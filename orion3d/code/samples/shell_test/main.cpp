...
#include "../../orion3d/shell/OR_ShellKernelManager.h"
#include "../../orion3d/shell/OR_ShellConsoleText.h"
...
//-------------------------------------------------------------------------
/** \name to handle the shell */
//-------------------------------------------------------------------------
//@{
#define KRD_FILE      "../krd/animation.krd"
//@}
...
void LoadLights()
{
	...
	pLight->SetName( OR_String("main_light") );
...
void LoadCameras()
{
	...
	pSphere->SetName( OR_String("cam_target") );
	...
	pCamera->SetName( OR_String("main_camera") );
...
void init(void)
{
	...
	OR_Mesh* pBox = (OR_Mesh*) pToolkit->GetByName( OR_MESH, "box" );
	if( pBox!=NULL )
	{
		/* assign a rigid body to the mesh */
		OR_RigidBody*	pRigidBody = new OR_RigidBody(*pBox);		
		pBox->SetRigidBody( *pRigidBody );		
		pToolkit->GetPhysicEntityManager().AddElement( pRigidBody );

		/* set the mesh as the root of the shell */
//		pToolkit->GetShellKernel().SetRootObject(*pBox);
	}
...
int main( int argc, char *argv[ ], char *envp[ ] )
{
	...
	GlWin=pInputOutput->Initialisation_Win32(NULL,NULL,0,"-==[ Test for shell ]==-",640,480,0,0);
	...
	pToolkit->OpenConsole();

	/* load all datas */
	init();

	/* launch a script */
 	pToolkit->ExecuteScript( "shell_test.lua", true );
