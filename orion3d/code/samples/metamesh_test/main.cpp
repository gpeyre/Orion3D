                       ...
                       int nCmdShow)
{
	...
	if( OR_Toolkit::GetVersion() < OR_ORION3D_Version )
		pToolkit->Warning("metamesh_test main", "The version of Orion3D.dll is not supported.");

	/* register the wrapper used for input.output */
	pToolkit->SetInputOutput(*pInputOutput);

	GlWin=pInputOutput->Initialisation_Win32(hInstance,NULL,0,"-==[ Test for metamesh ]==-",640,480,0,0);
